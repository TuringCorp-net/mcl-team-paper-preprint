...
The Meta-Coordination Layer (MCL) conceptual framework proposes that human organizational practices---role specialization, peer review, and deliberation-to-convergence---can be operationalized in multi-agent systems to improve output quality. This paper presents MCL-Team, a minimal realization of MCL, and evaluates it on LiveBench and IFEval, two public, contamination-resistant benchmarks. The Team algorithm consists of a three-stage pipeline: parallel generation by heterogeneous models, optional cross-reference correction, and fusion. Across three quality-cost tiers (Junior, Senior, Principal), MCL-Team achieves Language (93.5--94.2) and Instruction Following (87.4--88.8) scores that surpass all single models on the LiveBench public leaderboard, while scaling Reasoning from 84.5 to 97.0. On IFEval, MCL-Team reaches 92.6\% prompt-level strict accuracy and 95.0\% instruction-level strict accuracy, 5--9 points above tested competitors across both metrics. Collaboration increases token usage (5--10$\times$) and latency (2--10$\times$), defining an explicit cost-quality trade-off. Results are cross-validated by independent LLM analysis from DeepSeek, Gemini, and ChatGPT, all of which converge on the same ranking and score-profile interpretation. We conclude that minimal multi-model collaboration, when structured under MCL principles, reliably surpasses single-model baselines on quality-sensitive tasks.
...
\item
  \textbf{Benchmark results}: Language (93.5--94.2) and IF (87.4--88.8) surpassing all single models on the LiveBench leaderboard; Reasoning scaled from 84.5 (Junior) to 90.8 (Senior) to 97.0 (Principal); IFEval prompt-level strict accuracy up to 92.6\% and instruction-level strict accuracy up to 95.0\%.
\item
...
\item \textbf{Endpoint}: An OpenAI-compatible chat completions API (\texttt{/v1/chat/completions}). Researchers may request an API key for reproduction and independent verification; see the public repository at \url{https://github.com/TuringCorp-net/turingcorp-llm} for access instructions.
...
All benchmark result packages (LiveBench and IFEval raw outputs for Junior, Senior, and Principal tiers), model cards documenting tier configurations, and evaluation methodology are publicly available at \url{https://github.com/TuringCorp-net/turingcorp-llm}. The paper LaTeX source is available at \url{https://github.com/TuringCorp-net/mcl-team-paper-preprint}. The MCL-Team API endpoint used for evaluation is an OpenAI-compatible chat completions API. Researchers may request an API key for independent reproduction and verification. Proprietary implementation components (prompts, model routing, workflow controls) are not included in the public repositories.
...
Key observations (best tier per dimension):
\begin{itemize}
\tightlist
\item \textbf{Reasoning 97.0} (Principal): 7.3 points above the best single-model reasoner (Claude 4.8 Opus, 89.7).
\item \textbf{Language 94.2} (Senior): 5.7 points above the best single model for language (Claude Fable 5, 88.5).
\item \textbf{IF 88.8} (Senior): 13.2 points above the best single model for instruction following (Gemini 3.5 Flash, 75.6).
\end{itemize}

\subsubsection{5.3 IFEval}\label{ifeval-results}

IFEval evaluates instruction following through 25 verifiable constraint types with fully programmatic scoring---no LLM judge is involved. We report two granularities:

\begin{itemize}
\tightlist
\item \textbf{Prompt-level accuracy}: The fraction of prompts where \textit{all} instructions in the prompt are satisfied. This is the strictest measure.
\item \textbf{Instruction-level accuracy}: The fraction of \textit{individual} instructions satisfied across all prompts. This captures partial compliance.
\end{itemize}

Each granularity has a strict and a loose variant (lenient matching for minor formatting deviations).

\begin{table}[H]
\centering
\caption{IFEval results across all three tiers. Prompt-level and instruction-level, strict and loose.}
\label{tab:ifeval}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\begin{table}[H]
\centering
\caption{IFEval comparison against published single-model scores.}
...
MCL-Team Senior achieves 92.6\% prompt-level strict accuracy---5.3 points above Claude Opus 4 (87.3\%), the best tested single-model competitor. MCL-Team Principal reaches 95.0\% instruction-level strict accuracy, meaning that on average, 95 out of 100 individual instructions are followed correctly. All results are programmatically scored with no LLM-judge bias.
...
\item \textbf{Math}: 75.5 $\rightarrow$ 80.4 (+4.9). Structured problem-solving benefits from seeing alternative solution paths.
...
\item IFEval prompt-level strict accuracy up to 92.6\% and instruction-level strict accuracy up to 95.0\%, 5--9 points above tested single-model competitors on a fully programmatic benchmark.
